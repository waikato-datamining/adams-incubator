...
/*
 * ScriptedConfigurationGenerator.java
...
public class ScriptedConfigurationGenerator
  ...
  public String globalInfo() {
    return 
	"A configuration generator that uses any scripting handler for "
	+ "generating the configuration with a script located in the specified file.";
  }
  ...
  protected String loadScriptObject() {
	...
	AbstractJOOQConfigurationGenerator.class, 
	m_ScriptFile, 
	m_ScriptOptions, 
  ...
  protected void check() {
    ...
    m_GeneratorObject = (AbstractJOOQConfigurationGenerator) m_ScriptObject;
    if (m_GeneratorObject == null)
      throw new IllegalStateException("Failed to load script?");

    m_GeneratorObject.setDatabaseConnection(getDatabaseConnection());
    m_GeneratorObject.setOutput(getOutput());
  }
