...
public class HadoopRunLogPanel
  ...
  protected Calendar calendar = DateUtils.getCalendar();
   ...
   */
  public HadoopRunLogPanel(WekaTaskMonitor tm) {
   ...
   */
  public HadoopRunLogPanel(WekaTaskMonitor tm, boolean logHidden) {
  ...
  /**
   * Sends the supplied message to the log area. The current date and timestamp will
   * be prepended.
   *
  ...
  public synchronized void logMessage(String message) {
    ...
    m_LogText.append(" "+(calendar.get(Calendar.YEAR)-2000)+"/"+month+"/"+calendar.get(Calendar.DATE)+" "+HadoopRunLogPanel.getTimestamp() + " " + message);
    weka.core.logging.Logger.log(weka.core.logging.Logger.Level.INFO, message);
  }
  
  /**
   * Send the supplied message without current time data,
   * because Hadoop already provided it.
   * @param message
   */
  public synchronized void logMsgWithNoTime(String message){
    if (m_First) {
      m_First = false;
    } else {
      m_LogText.append("\n");
  ...
  public static void main(String [] args) {
    ...
    try {
      ...
      jf.getContentPane().setLayout(new BorderLayout());
      final HadoopRunLogPanel lp = new HadoopRunLogPanel();
