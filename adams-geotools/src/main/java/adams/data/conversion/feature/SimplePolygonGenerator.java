 ...
 * &nbsp;&nbsp;&nbsp;The range of columns to add to the features as well.
 * &nbsp;&nbsp;&nbsp;default: 
 * &nbsp;&nbsp;&nbsp;example: A range is a comma-separated list of single 1-based indices or sub-ranges of indices ('start-end'); 'inv(...)' inverts the range '...'; apart from column names (case-sensitive), the following placeholders can be used as well: first, second, third, last_2, last_1, last
 * </pre>
 * 
...
public class SimplePolygonGenerator
  ...
  public String globalInfo() {
    return 
	"Generates a polygon feature layer, using the GPS coordinates columns in the "
	+ "spreadsheet (at least three columns required) and adds any additionally "
  ...
  public String GPSTipText() {
    return "The range of the columns containing the GPS objects for the line (at least three columns required).";
  }
  ...
  public String additionalAttributesTipText() {
    return "The range of columns to add to the features as well.";
  }
  ...
  protected void check(SpreadSheet sheet) {
    ...
    if (m_GPS.getIntIndices().length == 0)
      throw new IllegalArgumentException("Columns with GPS objects not found: " + m_GPS.getRange());
    if (m_GPS.getIntIndices().length < 3)
      throw new IllegalArgumentException("Requires at least three columns with GPS objects: " + m_GPS.getRange());
  }
  ...
  protected SimpleFeature doGenerateFeature(Row row) {
    ...
    coord.add((Coordinate) coord.get(0).clone());
    
    m_Builder.add(m_GeometryFactory.createPolygon(coord.toArray(new Coordinate[coord.size()])));
